...
void Widget::setAlertText(string alertName, double value){
    QTableWidgetItem* item = alertItemMap.value(alertName);
    if (item == null){
        log(QString("no such alert ").append(QString::fromStdString(alertName)));
        return;
    }
    if (value == 0){
...
void Widget::display()
{
    ...
    // 单体过压报警
    setAlertText("dtgybj", dataPool->getDoubleByIndex(currentClusterIndex, "dtgy_bj"));
    // 单体欠压报警
    setAlertText("dtqybj", dataPool->getDoubleByIndex(currentClusterIndex, "dtqy_bj"));
    // 系统过压报警
    setAlertText("xtgybj", dataPool->getDoubleByIndex(currentClusterIndex, "xtgy_bj"));
    // 系统欠压报警
    setAlertText("xtqybj", dataPool->getDoubleByIndex(currentClusterIndex, "xtqy_bj"));
    // 系统压差报警
    setAlertText("xtycbj", dataPool->getDoubleByIndex(currentClusterIndex, "xtyc_bj"));
    // 系统温差报警
    setAlertText("xtwcbj", dataPool->getDoubleByIndex(currentClusterIndex, "xtwc_bj"));
    // 系统过温报警
    setAlertText("xtgwbj", dataPool->getDoubleByIndex(currentClusterIndex, "xtgw_bj"));
    // 系统低温报警
    setAlertText("xtdwbj", dataPool->getDoubleByIndex(currentClusterIndex, "xtdw_bj"));
    // 放电过流报警
    setAlertText("fdglbj", dataPool->getDoubleByIndex(currentClusterIndex, "fdgl_bj"));
    // 充电过流报警
    setAlertText("cdglbj", dataPool->getDoubleByIndex(currentClusterIndex, "cdgl_bj"));
    // SOC高报警
    setAlertText("socgbj", dataPool->getDoubleByIndex(currentClusterIndex, "socg_bj"));
    // SOC低报警
    setAlertText("socdbj", dataPool->getDoubleByIndex(currentClusterIndex, "socd_bj"));
    // 系统绝缘报警
    setAlertText("xtjybj", dataPool->getDoubleByIndex(currentClusterIndex, "xtjy_bj"));
    // 极柱高温报警
    setAlertText("jzgwbj", dataPool->getDoubleByIndex(currentClusterIndex, "jzgw_bj"));
    // 数据自检故障
    setAlertText("sjzjgz", dataPool->getDoubleByIndex(currentClusterIndex, "sjzj_gz"));
    // 与整车通信故障
    setAlertText("yzctxgz", dataPool->getDoubleByIndex(currentClusterIndex, "yzctx_gz"));
    // 与bmu通信故障
    setAlertText("ybmutxgz", dataPool->getDoubleByIndex(currentClusterIndex, "ybmutx_gz"));
    // 与充电机通信故障
    setAlertText("ycdjtxgz", dataPool->getDoubleByIndex(currentClusterIndex, "ycdjtx_gz"));
    // 加热故障
    setAlertText("jrgz", dataPool->getDoubleByIndex(currentClusterIndex, "jr_gz"));
    // 风扇故障
    setAlertText("fsgz", dataPool->getDoubleByIndex(currentClusterIndex, "fs_gz"));
    // 主正粘连故障
    setAlertText("zzzlgz", dataPool->getDoubleByIndex(currentClusterIndex, "zzzl_gz"));
    // 主负粘连故障
    setAlertText("zfzlgz", dataPool->getDoubleByIndex(currentClusterIndex, "zfzl_gz"));
    // 预充故障
    setAlertText("ycgz", dataPool->getDoubleByIndex(currentClusterIndex, "yc_gz"));
    // 电压传感器故障
    setAlertText("dycgqgz", dataPool->getDoubleByIndex(currentClusterIndex, "dycgq_gz"));
    // 温度传感器故障
    setAlertText("wdcgqgz", dataPool->getDoubleByIndex(currentClusterIndex, "wdcgq_gz"));
